...
function ApproveCV() {
    ...
    const handleSearch = () => {
        ...
        if (selectedFilters.school) {
            results = results.filter(intern => intern.school === selectedFilters.school);
        }
        if (selectedFilters.internID) {
            const searchText = selectedFilters.internID.toLowerCase();
            results = results.filter(intern => intern.internID.toLowerCase().includes(searchText));
        }
        if (selectedFilters.fullName) {
            const searchText = selectedFilters.fullName.toLowerCase();
            results = results.filter(intern => intern.fullName.toLowerCase().includes(searchText));
        }
        ...
        if (selectedFilters.address) {
            ...
            results = results.filter(intern => intern.address.toLowerCase().includes(searchText));
        }
        if (selectedFilters.phoneNumber) {
            const searchText = selectedFilters.phoneNumber.toLowerCase();
            results = results.filter(intern => intern.phoneNumber.toLowerCase().includes(searchText));
        }
        ...
        if (selectedFilters.dateSubmittedForm) {
            results = results.filter(intern =>
                moment(intern.dateSubmittedForm).isBetween(selectedFilters.dateSubmittedForm, 'day')
            );
        }
                        ...
                        <section className="filter-section">
                            <div className="filter">
                                <div className="fields">
                                    <Input size="large" placeholder="Enter intern's ID" value={selectedFilters.internID} onChange={(e) => handleInputChange('internID', e.target.value)} />

                                    <Input size="large" placeholder="Enter intern's Full name" value={selectedFilters.fullName} onChange={(e) => handleInputChange('fullName', e.target.value)} />

                                    <DatePicker format={dateFormat} placeholder="Enter intern's D.O.B" style={{ padding: "7px 11px", fontSize: "15px" }} onChange={(date) => handleDateChange('dateOfBirth', date)} />

                                    <Input size="large" placeholder="Enter intern's Phone number" value={selectedFilters.phoneNumber} onChange={(e) => handleInputChange('phoneNumber', e.target.value)} />
                                    ...
                                    <Dropdown overlay={createMenu('school', schoolNames)} trigger={['click']}>
                                        <Button style={{ padding: "7px 11px", fontSize: "15px", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center", height: "100%" }} >
                                            ...
                                            <div style={{ color: selectedFilters.school ? "#000000" : "#C7BFBF" }}>{selectedFilters.school || "Enter intern's School"}</div>
                                            <DownOutlined />
                                        </Button>
                                    </Dropdown>

                                    <Input size="large" placeholder="Enter intern's Email" value={selectedFilters.email} onChange={(e) => handleInputChange('email', e.target.value)} />
                                    ...
                                    <Input size="large" placeholder="Enter intern's Address" value={selectedFilters.address} onChange={(e) => handleInputChange('address', e.target.value)} />

                                    <DatePicker format={dateFormat} placeholder="Enter intern's Date Submitted Form" style={{ padding: "7px 11px", fontSize: "15px" }} onChange={(date) => handleDateChange('dateSub', date)} />
                                </div>
